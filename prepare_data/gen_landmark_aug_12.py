...
def GenerateData(ftxt, output,net,argument=False):
    ...
    for (imgPath, bbox, landmarkGt) in data:
        ...
        F_imgs = []
        F_landmarks = []
        path = os.path.join(data_path, imgPath.split('\\')[0] + '/' + imgPath.split('\\')[1])
        img = cv2.imread(path)
        ...
        if argument:
            ...
            for i in range(10):
                ...
                #rotate
                    ...
                    # if random.choice([0, 1]) > 0:
                    #     face_rotated_by_alpha, landmark_rotated = rotate(img, bbox,
                    #                                                      bbox.reprojectLandmark(landmark_), 135)  # 逆时针旋转
                    #     # landmark_offset
                    #     landmark_rotated = bbox.projectLandmark(landmark_rotated)
                    #     face_rotated_by_alpha = cv2.resize(face_rotated_by_alpha, (size, size), interpolation=cv2.INTER_LINEAR)
                    #     F_imgs.append(face_rotated_by_alpha)
                ...
                #inverse clockwise rotation
                    ...
                    # if random.choice([0, 1]) > 0:
                    #     face_rotated_by_alpha, landmark_rotated = rotate(img, bbox,
                    #                                                      bbox.reprojectLandmark(landmark_), 180)  # 逆时针旋转
                    #     # landmark_offset
                    #     landmark_rotated = bbox.projectLandmark(landmark_rotated)
                    #     face_rotated_by_alpha = cv2.resize(face_rotated_by_alpha, (size, size), interpolation=cv2.INTER_LINEAR)
                    #     F_imgs.append(face_rotated_by_alpha)
...
if __name__ == '__main__':
    net = "PNet"
    train_txt = "../data/trainImageList.txt"
