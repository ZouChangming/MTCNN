...
def save_hard_example(net, data,save_path):
    ...
    for im_idx, dets, gts in zip(im_idx_list, det_boxes, gt_boxes_list):
        ...
        for box in dets:
            ...
            if np.max(Iou) < 0.3 and neg_num < 80:
                #save the examples
        ...
        #         face_rotated = rotate(img, gt_box, 5)
        #         if np.shape(face_rotated)[0] > 0 and np.shape(face_rotated)[1] > 0:
        #             save_file = os.path.join(pos_dir, "%s.jpg" % p_idx)
        #             pos_file.write(save_file + ' 1 %.2f %.2f %.2f %.2f\n' % (0, 0, 0, 0))
        #             face_rotated = cv2.resize(face_rotated, (image_size, image_size), interpolation=cv2.INTER_LINEAR)
        #             cv2.imwrite(save_file, face_rotated)
        #             p_idx += 1
        #
        #     if random.choice([0, 1]) > 0:
        #         face_rotated = rotate(img, gt_box, 15)
        #         if np.shape(face_rotated)[0] > 0 and np.shape(face_rotated)[1] > 0:
        #             save_file = os.path.join(pos_dir, "%s.jpg" % p_idx)
        #             pos_file.write(save_file + ' 1 %.2f %.2f %.2f %.2f\n' % (0, 0, 0, 0))
        #             face_rotated = cv2.resize(face_rotated, (image_size, image_size), interpolation=cv2.INTER_LINEAR)
        ...
        #
        #     if random.choice([0, 1]) > 0:
        #         face_rotated = rotate(img, gt_box, 45)
        #         if np.shape(face_rotated)[0] > 0 and np.shape(face_rotated)[1] > 0:
        #             save_file = os.path.join(pos_dir, "%s.jpg" % p_idx)
        #             pos_file.write(save_file + ' 1 %.2f %.2f %.2f %.2f\n' % (0, 0, 0, 0))
        #             face_rotated = cv2.resize(face_rotated, (image_size, image_size), interpolation=cv2.INTER_LINEAR)
        ...
        #
        #     if random.choice([0, 1]) > 0:
        #         face_rotated = rotate(img, gt_box, 90)
        #         if np.shape(face_rotated)[0] > 0 and np.shape(face_rotated)[1] > 0:
        #             save_file = os.path.join(pos_dir, "%s.jpg" % p_idx)
        #             pos_file.write(save_file + ' 1 %.2f %.2f %.2f %.2f\n' % (0, 0, 0, 0))
        #             face_rotated = cv2.resize(face_rotated, (image_size, image_size), interpolation=cv2.INTER_LINEAR)
        ...
        #
        #     if random.choice([0, 1]) > 0:
        #         face_rotated = rotate(img, gt_box, 135)
        #         if np.shape(face_rotated)[0] > 0 and np.shape(face_rotated)[1] > 0:
        #             save_file = os.path.join(pos_dir, "%s.jpg" % p_idx)
        #             pos_file.write(save_file + ' 1 %.2f %.2f %.2f %.2f\n' % (0, 0, 0, 0))
        #             face_rotated = cv2.resize(face_rotated, (image_size, image_size), interpolation=cv2.INTER_LINEAR)
        ...
        #
        #     if random.choice([0, 1]) > 0:
        #         face_rotated = rotate(img, gt_box, 180)
        #         if np.shape(face_rotated)[0] > 0 and np.shape(face_rotated)[1] > 0:
        #             save_file = os.path.join(pos_dir, "%s.jpg" % p_idx)
        #             pos_file.write(save_file + ' 1 %.2f %.2f %.2f %.2f\n' % (0, 0, 0, 0))
        #             face_rotated = cv2.resize(face_rotated, (image_size, image_size), interpolation=cv2.INTER_LINEAR)
...
def t_net(prefix, epoch,
             ...
             thresh=[0.6, 0.7, 0.8], min_face_size=25,
...
def parse_args():
    ...
    parser.add_argument('--prefix', dest='prefix', help='prefix of model name', nargs="+",
                        default=['../data/MTCNN_model/PNet/model', '../data/MTCNN_model/RNet/model', '../data/MTCNN_model/ONet/model'],
                        type=str)
    ...
    parser.add_argument('--thresh', dest='thresh', help='list of thresh for pnet, rnet, onet', nargs="+",
                        default=[0.4, 0.5, 0.7], type=float)
