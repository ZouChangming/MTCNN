...
def train(net_factory, prefix, end_epoch, base_dir,
    ...
    net = prefix.split('/')[-2]
    #label file
    label_file = os.path.join(base_dir,'train_%s.txt' % net)
    ...
    if net == 'PNet':
        ...
        dataset_dir = os.path.join(base_dir, 'train_%s.tfrecord' % net)
        print dataset_dir
    ...
    #RNet use 3 tfrecords to get data    
        ...
        pos_dir = os.path.join(base_dir, 'pos_%s.tfrecord' % net)
        part_dir = os.path.join(base_dir, 'part_%s.tfrecord' % net)
        neg_dir = os.path.join(base_dir, 'neg_%s.tfrecord' % net)
        landmark_dir = os.path.join(base_dir, 'landmark_%s.tfrecord' % net)
    ...
    try:
        for step in range(MAX_STEP):
            ...
            if i * config.BATCH_SIZE > num*2:
                epoch = epoch + 1
                i = 0
                saver.save(sess, prefix + 'model', global_step=epoch*2)
